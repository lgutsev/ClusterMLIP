...
def summarize_evaluation(frames: list[LabeledFrame], predictions: list[Prediction]) -> EvaluationSummary:
    """Implements the README's "energy and force errors by charge and
    multiplicity" validation priority.
    ...
    Only this one validation priority is implemented. Isomer ordering,
    held-out reaction-family/cluster-size splits, barrier errors, and
    fragmentation-curve checks from the same README section are not: they
    need reaction-family/isomer-group labels and TS/IRC pairing this
    ...
    return {"n_frames": len(frames), "overall": overall, "by_charge_multiplicity": by_group}
...
def write_evaluation_report(
    ...
    for row in summary["by_charge_multiplicity"]:
        lines.append(
    ...
    lines += [
        "",
        "Only \"energy and force errors by charge and multiplicity\" from the README's "
        "Validation priorities is computed here. Isomer ordering, held-out reaction "
        "families/cluster sizes, barrier errors, fragmentation curves, and broken-symmetry "
        "root checks still need to be built.",
    ]
